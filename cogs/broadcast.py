...
class RoleSelectForBroadcast(discord.ui.View):
    ...
    def __init__(self, guild: discord.Guild, author: discord.Member):
        ...
        # Берём роли сервера (до 25 штук, исключаем @everyone)
        roles = [r for r in guild.roles if r.name != "@everyone"][:24]
        options = [
            discord.SelectOption(
                label=role.name[:90],
                value=str(role.id),
                description=f"{len(role.members)} участников",
            )
            for role in roles
        ]

        select = discord.ui.Select(
            placeholder="Выбери роль для рассылки...",
            options=options,
            custom_id="broadcast_role_select",
        )
